                    ...
                    const float scale)
{
    ...
    for(int h = 0; h < img_h; h++)
    {
        for(int w = 0; w < img_w; w++)
        {
            for(int c = 0; c < 3; c++)
            {
		input_data[c * hw + h * img_w + w] = (img.data[c * hw + h * img_w + w] - mean[c]) * scale;
            }
        }
    }
}
                         ...
                         const int img_h, const int img_w, const float scale, const int repeat_count)
{
    ...
    else if(model_format == "caffe_single" || model_format == "onnx" || model_format == "tensorflow" || model_format == "tengine")
    {
        model_fnum = 1;
        if(model_file.empty())
        {
